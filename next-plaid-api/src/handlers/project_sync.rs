...
use crate::handlers::documents::get_index_lock_by_path;
use crate::handlers::encode::encode_texts_internal;
use crate::models::{
    ErrorResponse, InputType, ProjectSyncCreateJobRequest, ProjectSyncCreateJobResponse,
    ProjectSyncJobResponse, ProjectSyncJobStatus,
};
...
fn max_project_sync_batch_documents() -> usize {
...
async fn inspect_spool_state(state: &AppState, job_id: &str) -> ApiResult<ProjectSyncSpoolState> {
    let final_bytes = match fs::metadata(spool_path(state, job_id)).await {
    ...
    let temp_bytes = match fs::metadata(temp_spool_path(state, job_id)).await {
        Ok(metadata) => Some(metadata.len()),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => None,
        Err(error) => {
            return Err(ApiError::Internal(format!(
                "Failed to inspect project_sync temp spool file: {}",
                error
            )))
        }
    };
...
fn recover_manifest_for_restart(
    ...
    let has_exact_final = spool_state.final_bytes == Some(manifest.declared_bytes);
    let has_exact_temp = spool_state.temp_bytes == Some(manifest.declared_bytes);
    let has_any_spool = spool_state.final_bytes.is_some() || spool_state.temp_bytes.is_some();
    ...
    match manifest.status {
        ...
        ProjectSyncJobStatus::Running => {
            manifest.status = ProjectSyncJobStatus::Failed;
            manifest.error = Some(
                "project_sync job was interrupted during indexing; manual retry required"
                    .to_string(),
            );
            manifest.reserved_bytes = false;
            ProjectSyncRecoveryAction::Persist {
        ...
        ProjectSyncJobStatus::Cancelling => {
            manifest.status = ProjectSyncJobStatus::Cancelled;
            manifest.error = Some("Cancelled during recovery".to_string());
            manifest.reserved_bytes = false;
            ProjectSyncRecoveryAction::Persist {
                manifest,
                requeue: false,
...
async fn recover_project_sync_job_dir(
    ...
    match recover_manifest_for_restart(manifest, spool_state, now_epoch_ms()) {
        ProjectSyncRecoveryAction::Persist {
            manifest,
            requeue,
            spool_fix,
...
pub async fn recover_project_sync_jobs(state: Arc<AppState>) -> ApiResult<()> {
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|error| {
        ...
        let (reserved_bytes, requeue) =
            recover_project_sync_job_dir(&state, &entry_name, &entry_path).await?;
        pending_bytes = pending_bytes.saturating_add(reserved_bytes);
        if requeue {
    ...
    for job_id in requeue_job_ids {
        ...
        tokio::spawn(async move {
            run_project_sync_job(worker_state, job_id).await;
        });
    }
...
async fn upload_project_sync_body(
    ...
    loop {
        if !ensure_project_sync_job_not_cancelled(&state, job_id).await? {
...
async fn ensure_project_sync_job_not_cancelled(state: &AppState, job_id: &str) -> ApiResult<bool> {
    let lock = job_lock(job_id);
    let _guard = lock.lock().await;
    let mut manifest = read_manifest(state, job_id).await?;
    if !is_cancellation_requested(manifest.status) {
...
async fn complete_project_sync_job(state: &AppState, job_id: &str) -> ApiResult<()> {
    let lock = job_lock(job_id);
    let _guard = lock.lock().await;
    let mut manifest = read_manifest(state, job_id).await?;
    if is_terminal_status(manifest.status) {
...
async fn run_project_sync_job_inner(state: Arc<AppState>, job_id: &str) -> ApiResult<()> {
    {
        ...
        manifest.status = ProjectSyncJobStatus::Running;
        manifest.error = None;
        manifest.updated_at_ms = now_epoch_ms();
        write_manifest(&state, &manifest).await?;
    }
    ...
    let path_str = state
        ...
        .to_string_lossy()
        .to_string();
    let index_lock = get_index_lock_by_path(&path_str);
    ...
    let spool = fs::File::open(spool_path(&state, job_id))
        .await
        .map_err(|error| {
            ApiError::Internal(format!("Failed to open project_sync spool: {}", error))
        })?;
    let mut lines = BufReader::new(spool).lines();
    let mut documents: Vec<String> = Vec::new();
    let mut metadata: Vec<serde_json::Value> = Vec::new();

    while let Some(line) = lines.next_line().await.map_err(|error| {
        ApiError::Internal(format!("Failed to read project_sync spool line: {}", error))
    })? {
        let record = parse_project_sync_record(&line)?;
        documents.push(record.content.clone());
        metadata.push(build_project_sync_metadata(&record)?);

        if documents.len() >= max_project_sync_batch_documents() {
            if !ensure_project_sync_job_not_cancelled(&state, job_id).await? {
                return Ok(());
            }
    ...
    if !documents.is_empty() {
        ...
        process_project_sync_batch_locked(state.clone(), &manifest.index_name, documents, metadata)
            .await?;
    }

    complete_project_sync_job(&state, job_id).await
}
...
async fn process_project_sync_batch_locked(
    ...
    let embeddings =
        encode_texts_internal(state.clone(), &documents, InputType::Document, None).await?;
    commit_project_sync_batch_locked(state, index_name, embeddings, metadata).await
}

async fn commit_project_sync_batch_locked(
    state: Arc<AppState>,
    index_name: &str,
    ...
    task::spawn_blocking(move || {
        commit_project_sync_batch_blocking(
            state_clone,
            &index_name_owned,
            &path_str,
            embeddings,
    ...
    .await
    .map_err(|error| ApiError::Internal(format!("project_sync batch task panicked: {}", error)))?
}

fn commit_project_sync_batch_blocking(
    state: Arc<AppState>,
    index_name: &str,
    path_str: &str,
    embeddings: Vec<Array2<f32>>,
    ...
    let stored_config = state
        ...
        .ok_or_else(|| ApiError::IndexNotFound(index_name.to_string()))?;
    ...
    let (mut index, doc_ids) =
        MmapIndex::update_or_create(&embeddings, path_str, &index_config, &update_config)
            .map_err(|error| ApiError::Internal(format!("Index update failed: {}", error)))?;

    let db_result = if filtering::exists(path_str) {
    ...
    if let Err(error) = db_result {
        if let Err(cleanup_error) = index.delete_with_options(&doc_ids, false) {
            tracing::error!(
                index = %index_name,
                error = %cleanup_error,
                operation = "metadata_failure_cleanup",
                "project_sync.metadata_failure_cleanup.failed"
            );
        }
        return Err(ApiError::Internal(format!(
            "Failed to update metadata during project_sync: {}",
            error
        )));
    }
        ...
        next_plaid::text_search::index(path_str, &metadata, &doc_ids, &fts_tokenizer)
    {
        tracing::warn!(
            index = %index_name,
            error = %error,
            "project_sync.fts_index.failed"
        );
    }

    if let Some(max_documents) = stored_config.max_documents {
        evict_oldest_documents(&mut index, max_documents)?;
    }

    state.reload_index(index_name)?;
    Ok(())
}
