...
async fn restore_project_sync_snapshot_locked(
    ...
    task::spawn_blocking(move || restore_project_sync_snapshot_blocking(&snapshot_path, &index_path))
    .await
...
fn restore_project_sync_snapshot_blocking(snapshot: &Path, index_path: &Path) -> Result<(), String> {
