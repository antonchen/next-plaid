...
fn to_ndarray(query: &QueryEmbeddings) -> ApiResult<Array2<f32>> {
    if let (Some(b64), Some(shape)) = (&query.embeddings_b64, &query.shape) {
        return crate::models::decode_b64_embeddings_to_array2(b64, *shape, "query")
...
fn execute_keyword_or_hybrid_search(
    ...
    for query_id in 0..num_queries {
        ...
        let keyword = if has_text_query {
            let result = if let Some(ref subset) = subset {
            ...
            Some(
                result
                    .map(|result| (result.passage_ids, result.scores))
...
pub async fn search(
    ...
    let trace_id = trace_id.map(|value| value.0).unwrap_or_default();
