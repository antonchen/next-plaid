...
fn build_model_instance(
    ...
    let mut builder = next_plaid_onnx::Colbert::builder(model_path)
        ...
        .with_quantized(use_int8);

    if let Some(value) = parallel_sessions {
    ...
    if let Some(value) = query_length {
        builder = builder.with_query_length(value);
    }
    if let Some(value) = document_length {
...
fn build_model_pool(
    ...
    let cached_info = state::CachedModelInfo {
        name: model_config.model_name().map(|value| value.to_string()),
        path: model_path.to_string_lossy().to_string(),
        quantized: use_int8,
        embedding_dim: model.embedding_dim(),
        batch_size: model.batch_size(),
        num_sessions: model.num_sessions(),
        query_prefix: model_config.query_prefix.clone(),
        document_prefix: model_config.document_prefix.clone(),
        query_length: model_config.query_length,
        document_length: model_config.document_length,
        do_query_expansion: model_config.do_query_expansion,
        uses_token_type_ids: model_config.uses_token_type_ids,
...
"#
    ...
    let state = {
        ...
        if let Some(ref loaded_model_path) = model_path {
            if _query_on_cpu {
                let ingest_model = match build_model_instance(
                    loaded_model_path,
                    true,
                    _use_int8,
                    _parallel_sessions,
                ...
                let query_model = match build_model_instance(
                    loaded_model_path,
                    false,
                    _use_int8,
                    _parallel_sessions,
                ...
                let query_pool = build_model_pool(
                    query_model,
                    loaded_model_path,
                    false,
                    _use_int8,
                ...
                let model = match build_model_instance(
                    loaded_model_path,
                    _use_cuda,
                    _use_int8,
                    _parallel_sessions,
                ...
                let model_pool = build_model_pool(
                    model,
                    loaded_model_path,
                    _use_cuda,
                    _use_int8,
